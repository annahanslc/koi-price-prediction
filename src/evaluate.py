...
def evaluate_model_flare(model, val_loader, device, class_labels):
    ...
    with torch.no_grad():
        for images, labels in val_loader:
            ...
            all_preds.append(
                preds.cpu()
            )  # moves predictions and labels from GPU to CPU
            all_labels.append(
                labels.cpu().int()
            )  # convert labels to int (for metrics like precision/recall)
    ...
    matches = (
        (all_preds_tensor == all_labels_tensor).all(dim=1).float()
    )  # checks if all labels match
    ...
    print(
        f"Validation Accuracy: {accuracy:.4f}, F1 Score: {f1:.4f}, Precision: {precision:.4f}, Recall: {recall:.4f}"
    )
